...
public class Application {
	...
	private void initClassLoader() throws Exception {
		if (log.isDebugEnabled()) {
			log.debug("{} initializing ClassLoader", this);
		}
		...
		loader.load();
		if (log.isDebugEnabled()) {
			log.debug("{} loader.loaded()", this);
		}

		classLoader = loader.getClassLoader();

		if (log.isDebugEnabled()) {
			log.debug("{} initialized ClassLoader");
		}
	}

	private void initConfig() throws Exception {
		if (log.isDebugEnabled()) {
			log.debug("{} initializing Config", this);
		}
		...
		//
		// set Classes Directory. Starting with release 0.3.0, can be configurable
		// relatively to Basedir.
		//
		...
		if (!classesdir.exists())
			throw new RuntimeException(
					"@Application " + name + "; classes directory (" + classesdir.getAbsolutePath() + ") not defined");
		config.setClassesDirectory(classesdir);

		//
		// Set Lib Directory. Starting with release 0.3.0, can be configurable
		// relatively to Basedir.
		File libdir = null;
		...
		// first entry is the application name... which also can be an alias to final
		// domain
		...
		if (domainAliases != null) {
			...
			for (int i = 0; i < ds.length; i++) {
				// domain name and aliases shall be always lowered case so facilitates case
				// sensitive searches
		...
		if (log.isDebugEnabled())
			log.debug("{} responsible for domains: {}", this, domains);
		...
		if (log.isDebugEnabled()) {
			log.debug("{} initialized config", this);
		}
	}

	private void initContext() throws Exception {
		if (log.isDebugEnabled()) {
			log.debug("{} initializing ApplicationContext", this);
		}
		...
		if (log.isDebugEnabled()) {
			log.debug("{} initialized ApplicationContext", this);
		}
	}
	...
	public void init() throws Exception {
		...
		messageFactory.init();
		if (log.isDebugEnabled()) {
			log.debug("{} initialized MessageFactory", this);
		}

		responderFactory = new ResponderFactory();
		responderFactory.init(this);
		if (log.isDebugEnabled()) {
			log.debug("{} initialized ResponderFactory", this);
		}
		...
		if (log.isDebugEnabled()) {
			log.debug("{} initialized with SessionFactory {}", this, sessionFactory);
		}
		...
		log.info("executing initializers of @Application {}", name);
		...
		log.info("init has finished @Application {}", name);
	}
	...
	public void start() throws Exception {
		...
		init();
		if (log.isInfoEnabled()) {
			log.info("@Application[{}].initializers: {}", this.name, loader.getInitializers().size());
			log.info("@Application[{}].interceptors: {}", this.name, loader.getInterceptors().size());
			log.info("@Application[{}].controllers: {}", this.name, loader.getControllers().size());
			log.info("@Application[{}] started", this.name);
		}
	}
